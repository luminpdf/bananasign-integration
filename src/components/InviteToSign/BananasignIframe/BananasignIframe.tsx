...
const BananasignIframe: React.FC = () => {
  ...
  const {
    state: {openBananasignIframe, onClose, bananasignUrl, flowId, accessToken},
    dispatch,
  } = context;
  ...
  if (!openBananasignIframe || !flowId || !accessToken) {
    return null;
  }
  ...
  return (
    <div className="BananasignIframe__container">
      <iframe
        width="100%"
        height="100%"
        src={`${bananasignUrl}/embed/${flowId}?token=${accessToken}`}
        title="Iframe Upload"
        frameBorder="0"
