...
export var InviteToSignContextReducer = function (state, action) {
    switch (action.type) {
        ...
        case INVITE_TO_SIGN_ACTION_TYPES.SET_OPEN_ADD_ASSIGNER_MODAL:
            return __assign(__assign({}, state), { isOpenAddAssignerModal: action.payload });
        case INVITE_TO_SIGN_ACTION_TYPES.REMOVE_SIGNER:
            var signers = __spreadArray([], state.signers, true);
            var newSigners = signers.filter(function (item) { return item.email !== action.payload.email; });
        ...
        case INVITE_TO_SIGN_ACTION_TYPES.REMOVE_VIEWER:
            ...
            var newViewers = viewers.filter(function (item) { return item.email !== action.payload.email; });
            return __assign(__assign({}, state), { viewers: newViewers });
        case INVITE_TO_SIGN_ACTION_TYPES.SET_WORD_SEARCH_CONTACT:
            return __assign(__assign({}, state), { keyWordSearchContact: action.payload });
        ...
        case INVITE_TO_SIGN_ACTION_TYPES.ADD_VIEWER:
            var viewerIndex = state.viewers.findIndex(function (viewer) { return viewer.email === action.payload.email; });
            if (viewerIndex > -1) {
                return __assign({}, state);
            }
            return __assign(__assign({}, state), { viewers: __spreadArray(__spreadArray([], state.viewers, true), [action.payload], false) });
        case INVITE_TO_SIGN_ACTION_TYPES.ADD_SIGNER:
            var signerIndex = state.signers.findIndex(function (signer) { return signer.email === action.payload.email; });
            if (signerIndex > -1) {
                return __assign({}, state);
            }
            return __assign(__assign({}, state), { signers: __spreadArray(__spreadArray([], state.signers, true), [action.payload], false) });
        ...
        case INVITE_TO_SIGN_ACTION_TYPES.CANCEL_ADD_ASSIGNERS:
            return __assign(__assign({}, state), { cancelAddAssigners: action.payload });
        ...
        case INVITE_TO_SIGN_ACTION_TYPES.CLOSE_AND_RESET_MODAL_SEARCH:
            return __assign(__assign({}, state), { searchContacts: [], keyWordSearchContact: '', isOpenAddAssignerModal: false });
        case INVITE_TO_SIGN_ACTION_TYPES.OPEN_BANANASIGN_IFRAME:
            return __assign(__assign({}, state), { openBananasignIframe: action.payload });
